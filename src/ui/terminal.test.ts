...
describe("KeyParser", () => {
  ...
  test("SGR mouse wheel", () => {
    // CSI < 6 4 ; 1 0 ; 5 M  => wheel up at col10,row5
    const up = [0x1b, 0x5b, 0x3c, 0x36, 0x34, 0x3b, 0x31, 0x30, 0x3b, 0x35, 0x4d];
    expect(events(up).map((e) => e.kind)).toEqual(["scrollup"]);
    const down = [0x1b, 0x5b, 0x3c, 0x36, 0x35, 0x3b, 0x31, 0x30, 0x3b, 0x35, 0x4d];
    expect(events(down).map((e) => e.kind)).toEqual(["scrolldown"]);
    const drag = [0x1b, 0x5b, 0x3c, 0x33, 0x32, 0x3b, 0x31, 0x30, 0x3b, 0x35, 0x4d];
    expect(events(drag).map((e) => e.kind)).toEqual(["unknown"]);
  });
